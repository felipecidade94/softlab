...
class SistemaLaboratorio:
    def __init__(self):  # Método inicializador da classe
        ...
        self.arquivo_dados = "SOFTLAB_DADOS.txt"  # Nome do arquivo para salvar os dados
        ...
        self.criar_wigets()  # Chama o método para criar os widgets da interface
        self.root.title("SOFTLAB")  # Define o título da janela
        self.texto_embaixo = Label(self.root, text="Por Felipe Cidade Soares e Ivy Oliveira dos Reis\nhttps://github.com/felipecidade94/SOFTLAB", font=("Arial", 10), fg="blue", anchor="w") # Cria um texto, à esquerda, na parte de baixo da janela
        self.texto_embaixo.pack(fill="x") # Posiciona o texto na horizontal
        self.root.geometry("400x640")  # Define o tamanho da janela
    ...
    def criar_wigets(self):  # Método para criar os widgets da interface
        style = ttk.Style()  # Cria um estilo para os widgets
        style.configure("TButton", padding=15, font=("Arial", 10), foreground="black", background="#add8e6")  # Configura o estilo dos botões cor azul claro
        style.map("TButton",background=[("active", '#00008B')],font=[("active", ("Arial", 11, "bold"))]) # Configura o estilo dos botões quando ativados cor azul escuro
        style.configure("TLabel", padding=10, font=("Arial", 20, "bold"))  # Configura o estilo do título
        ...
        self.label_title.grid(row=0, column=0, columnspan=2, pady=1)  # Posiciona o rótulo no grid
        self.btn_registrar = ttk.Button(self.frame, text="REGISTRAR NOVO(A) ALUNO(A)", command=self.registrar_aluno, style="TButton", width=40)  # Cria o botão de registrar aluno
        self.btn_registrar.grid(row=1, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_listar = ttk.Button(self.frame, text="LISTAR ALUNOS(AS)", command=self.listar_alunos, style="TButton", width=40)  # Cria o botão de listar alunos
        self.btn_listar.grid(row=2, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_login = ttk.Button(self.frame, text="FAZER LOGIN", command=self.fazer_login, style="TButton", width=40)  # Cria o botão de login
        self.btn_login.grid(row=3, column=0, pady=5, padx=5)  # Posiciona o botão no grid 
        self.btn_adicionar_projeto = ttk.Button(self.frame, text="ADICIONAR ATIVIDADE", command=self.adicionar_projeto, style="TButton", width=40)  # Cria o botão de adicionar projeto
        self.btn_adicionar_projeto.grid(row=4, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_listar_projetos = ttk.Button(self.frame, text="LISTAR ATIVIDADES DO(A) ALUNO(A)", command=self.listar_projetos_aluno, style="TButton", width=40)  # Cria o botão de listar projetos
        self.btn_listar_projetos.grid(row=5, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_alterar = ttk.Button(self.frame, text="ALTERAR DADOS DO(A) ALUNO(A)", command=self.alterar_dados_aluno, style="TButton", width=40)  # Cria o botão de alterar dados do aluno
        self.btn_alterar.grid(row=6, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_remover = ttk.Button(self.frame, text="REMOVER ALUNO(A)", command=self.remover_aluno, style="TButton", width=40)  # Cria o botão de remover aluno
        self.btn_remover.grid(row=7, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_sair = ttk.Button(self.frame, text="SAIR", command=self.fechar_janela, style="TButton", width=40)  # Cria o botão de sair
    ...
    def registrar_aluno(self):  # Método para registrar um novo aluno
        try:
            ...
            if matricula is None or matricula == "" or not matricula.isnumeric():
                raise ValueError
            ...
            messagebox.showerror("Erro",str("Informe um número de matrícula. Apenas números são permitidos."))      
        else:
            ...
            try:
                ...
                if nome == "" or nome is None or not nome.replace(" ","").isalpha():
                    raise ValueError
                else:
                    ...
                    messagebox.showerror("Erro",str("Informe o nome do(a) aluno(a). Apenas letras são permitidas."))
    ...
    def adicionar_projeto(self):  # Método para adicionar um projeto a um aluno
        ...
        projeto_escolhido_idx = simpledialog.askinteger("Adicionar atividade", f"Escolha uma atividade pelo número:\n{lista_projetos}")  # Solicita o número do projeto escolhido
    ...
    def listar_projetos_aluno(self):  # Método para listar os projetos de um aluno
        ...
        matricula = self.usuario_logado  # Obtém a matrícula do usuário logado
        if matricula not in self.projetos:  # Verifica se a matrícula não tem projetos
            messagebox.showinfo("Informação", "O(a) aluno(a) não possui projetos cadastrados.")  # Exibe uma mensagem informativa
            return
        lista_projetos = "\n".join(self.projetos[matricula])  # Cria a lista de projetos do aluno
        messagebox.showinfo("Atividades do aluno", f"Atividade(s) do(a) aluno(a) {self.alunos[matricula]}:\n{lista_projetos}")  # Exibe a lista de projetos
    ...
    def alterar_dados_aluno(self):  # Método para alterar os dados de um aluno
        ...
        matricula = self.usuario_logado  # Obtém a matrícula do usuário logado
        if matricula not in self.alunos:  # Verifica se a matrícula não está cadastrada
            messagebox.showerror("Erro", "Aluno(a) não encontrado.")  # Exibe uma mensagem de erro
            return
        novo_nome = simpledialog.askstring("Alterar dados de um aluno(a)", f"Digite o novo nome para o aluno(a) {self.alunos[matricula]}:")  # Solicita o novo nome do aluno
        if novo_nome:
            self.alunos[matricula] = novo_nome  # Atualiza o nome do aluno no dicionário
            self.salvar_dados()  # Salva os dados no arquivo
            messagebox.showinfo("Sucesso", "Dados do(a) aluno(a) alterados com sucesso!")  # Exibe uma mensagem de sucesso
        else:
            messagebox.showinfo("Informação", "Operação de alteração de dados cancelada.")  # Exibe uma mensagem informativa
    ...
    def remover_aluno(self):  # Método para remover um aluno
        ...
        if matricula not in self.alunos:  # Verifica se a matrícula não está cadastrada
            messagebox.showerror("Erro", "Aluno(a) não encontrado.")  # Exibe uma mensagem de erro
            return
        confirmar = messagebox.askyesno("Remover aluno(a)", f"Tem certeza que deseja remover o(a) aluno(a) {self.alunos[matricula]}?")  # Solicita confirmação para remover o aluno
        if confirmar:
            ...
            self.salvar_dados()  # Salva os dados no arquivo
            messagebox.showinfo("Sucesso", "Aluno(a) removido(a) com sucesso!")  # Exibe uma mensagem de sucesso
        else:
