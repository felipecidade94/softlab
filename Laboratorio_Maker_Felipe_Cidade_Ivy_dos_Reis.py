...
class SistemaLaboratorio:
    def __init__(self):  # Método inicializador da classe
        ...
        self.root.geometry("400x620")  # Define o tamanho da janela
    ...
    def create_widgets(self):  # Método para criar os widgets da interface
        ...
        style.configure("TButton", padding=14, font=("Arial", 10), foreground="black", background="#add8e6")  # Configura o estilo dos botões
        ...
        self.btn_registrar = ttk.Button(self.frame, text="REGISTRAR NOVO ALUNO", command=self.registrar_aluno, style="TButton", width=35)  # Cria o botão de registrar aluno
        self.btn_registrar.grid(row=1, column=0, pady=5, padx=15)  # Posiciona o botão no grid
        self.btn_login = ttk.Button(self.frame, text="FAZER LOGIN", command=self.fazer_login, style="TButton", width=35)  # Cria o botão de login
        self.btn_login.grid(row=2, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_listar = ttk.Button(self.frame, text="LISTAR ALUNOS(AS)", command=self.listar_alunos, style="TButton", width=35)  # Cria o botão de listar alunos
        self.btn_listar.grid(row=3, column=0, pady=5, padx=5)  # Posiciona o botão no grid
        self.btn_adicionar_projeto = ttk.Button(self.frame, text="ADICIONAR ATIVIDADE", command=self.adicionar_projeto, style="TButton", width=35)  # Cria o botão de adicionar projeto
        self.btn_adicionar_projeto.grid(row=4, column=0, pady=5, padx=10)  # Posiciona o botão no grid
        self.btn_listar_projetos = ttk.Button(self.frame, text="LISTAR ATIVIDADES DO(A) ALUNO(A)", command=self.listar_projetos_aluno, style="TButton", width=35)  # Cria o botão de listar projetos

        self.btn_listar_projetos.grid(row=5, column=0, pady=5, padx=5)  # Posiciona o botão no grid

        self.btn_alterar = ttk.Button(self.frame, text="ALTERAR DADOS DO(A) ALUNO(A)", command=self.alterar_dados_aluno, style="TButton", width=35)  # Cria o botão de alterar dados do aluno
        
        self.btn_alterar.grid(row=6, column=0, pady=5, padx=5)  # Posiciona o botão no grid

        self.btn_remover = ttk.Button(self.frame, text="REMOVER ALUNO(A)", command=self.remover_aluno, style="TButton", width=35)  # Cria o botão de remover aluno
        self.btn_remover.grid(row=7, column=0, pady=5, padx=5)  # Posiciona o botão no grid

        self.btn_sair = ttk.Button(self.frame, text="SAIR", command=self.fechar_janela, style="TButton", width=35)  # Cria o botão de sair
    ...
    def listar_projetos_aluno(self):  # Método para listar os projetos de um aluno
        ...
        messagebox.showinfo("Atividades do(a) aluno(a)", f"Atividades de {self.alunos[matricula]}:\n{lista_projetos}")  # Exibe a lista de projetos
    ...
    def remover_aluno(self):  # Método para remover um aluno
        ...
        matricula = self.usuario_logado  # Obtém a matrícula do usuário logado
        if matricula not in self.alunos:  # Verifica se a matrícula não está cadastrada
            messagebox.showerror("Erro", "Aluno(a) não encontrado(a).")  # Exibe uma mensagem de erro
            return

        confirmar = messagebox.askyesno("Remover aluno(a)", f"Tem certeza que deseja remover o aluno(a) {self.alunos[matricula]}?")  # Solicita confirmação para remover o aluno
        if confirmar:
            ...
            self.salvar_dados()  # Salva os dados no arquivo
            messagebox.showinfo("Sucesso", "Aluno(a) removido com sucesso!")  # Exibe uma mensagem de sucesso
        else:
